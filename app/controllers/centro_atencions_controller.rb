...
class CentroAtencionsController < ApplicationController
  ...
  def new
    ...
    #@centro_atencion.servicio
    if(@centro_atencion.servicio != nil)
      @categoria = @b.classify :servicio
    end
    if(@centro_atencion.servicio == nil)
      @categoria = @b.classify ""
    end    
  ...
  private
    ...
    def centro_atencion_params
      @prueba = params.require(:centro_atencion).permit(:categoria, :servicio)
    end
end
